...
public class LogisticsServiceImpl extends BaseRestClientService implements LogisticsService {
    ...
    public List<Logistics> getLogistics(BaseRequest baseRequest) {
        ...
        return post(LogisticsList.class, uri(LOGISTICS_GET_RELATIVE_PATH)).entity(baseRequest).execute().getList();
    }
    ...
    public ParameterForInit getParametersForInit(LogisticsIdRequest request) {
        ...
        return post(ParameterForInit.class, uri(LOGISTICS_GET_PARAMETER_INIT_RELATIVE_PATH)).entity(request).execute();
    }

    @Override
    public List<Address> getAddresses(BaseRequest baseRequest) {
        checkNotNull(baseRequest);
        return post(Addresses.class, uri(LOGISTICS_GET_ADDRESS_RELATIVE_PATH)).entity(baseRequest).execute().getList();
    }
    ...
    public List<PickupTime> getTimeSlot(LogisticsIdRequest request) {
        ...
        return post(PickupTimeSlots.class, uri(LOGISTICS_GET_TIMESLOT_RELATIVE_PATH)).entity(request).execute().getList();
    ...
    public List<Branch> getBranches(LogisticsIdRequest request) {
        ...
        return post(Branches.class, uri(LOGISTICS_GET_BRANCH_RELATIVE_PATH)).entity(request).execute().getList();
    }
    ...
    public LogisticsInfo getLogisticsInfo(LogisticsIdRequest request) {
        checkNotNull(request);
        checkArgument(!StringUtils.isEmpty(request.getOrdersn()));
        return post(LogisticsInfo.class, uri(LOGISTICS_GET_INFO_RELATIVE_PATH)).entity(request).execute();
    }
    ...
    public AirwayBillsResult getAirwayBills(LogisticsIdsRequest idsRequest) {
        ...
        return post(AirwayBillsResult.class, uri(LOGISTICS_GET_AIRWAY_BILL_RELATIVE_PATH)).entity(idsRequest).execute();
    }
    ...
    public OrderLogistics getOrderLogistics(LogisticsIdRequest request) {
        checkNotNull(request);
        checkArgument(!StringUtils.isEmpty(request.getOrdersn()));
        return post(OrderLogisticsWrapper.class, uri(LOGISTICS_GET_ORDER_LOGISTICS_RELATIVE_PATH)).entity(request).execute().get(OrderLogistics.class);
    }
    ...
    public TrackingInfoResult getLogisticsMessage(LogisticsIdRequest request) {
        checkNotNull(request);
        checkArgument(!StringUtils.isEmpty(request.getOrdersn()));
        return post(TrackingInfoResult.class, uri(LOGISTICS_GET_TRACKING_RELATIVE_PATH)).entity(request).execute();
    }
}
